<template>
  ...
  >
    ...
    <div class="absolute inset-0 overflow-hidden pointer-events-none">
      <div class="absolute top-10 left-1/4 w-72 h-72 bg-primary/5 rounded-full blur-3xl animate-float"></div>
      <div class="absolute bottom-0 right-1/4 w-96 h-96 bg-purple/5 rounded-full blur-3xl animate-float" style="animation-delay: -3s;"></div>
    </div>

    <div class="relative max-w-4xl mx-auto px-4 sm:px-6 text-center">
      <template v-if="showSlogan">
        <div class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full glass text-sm text-text-secondary"
        ...
        <h1 :class="compact ? 'text-2xl sm:text-3xl mb-2' : 'text-3xl sm:text-5xl mb-4'" class="font-bold text-text-primary leading-tight">
          免费在线视频解析器
          <span class="bg-gradient-to-r from-primary to-purple bg-clip-text text-transparent">，一键保存</span>
        </h1>
      ...
      <div class="max-w-2xl mx-auto">
        <form @submit.prevent="onSubmit" class="relative flex items-center" role="search" aria-label="视频链接解析">
          <div class="relative flex-1">
            ...
            <input
              ...
              class="w-full h-13 sm:h-14 pl-12 pr-4 rounded-full sm:rounded-r-none border border-border bg-bg-card text-base text-text-primary placeholder:text-text-muted input-glow transition-all"
              :disabled="loading"
              autocomplete="url"
            />
          </div>
          <button
            ...
            class="hidden sm:flex items-center gap-2 h-14 px-8 rounded-r-full btn-primary text-white font-medium text-base disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
          >
          ...
          <button
            type="submit"
            :disabled="loading || !url.trim()"
            class="sm:hidden absolute right-2 top-1/2 -translate-y-1/2 w-9 h-9 flex items-center justify-center rounded-full bg-gradient-to-r from-primary to-purple text-white disabled:opacity-50 cursor-pointer"
          >
        ...
        <div v-if="showSlogan" class="flex flex-wrap items-center justify-center gap-3 mt-5 text-xs text-text-muted">
          ...
          <button
            ...
            class="px-3 py-1 rounded-full bg-bg-card border border-border hover:border-primary/50 hover:text-primary transition-all cursor-pointer"
          >
            {{ example.label }}
          </button>
        </div>
